...
void equalizer::display_eq(void)
{
	...
	cout << "CMD ParametricEQ v0.0.1 " << endl;
	cout << "Follow me on GitHub: https://github.com/henrikleod" << endl;
	cout << "--------------------------------------------------------------" << endl;
	cout << "   Pre  70   180  320  600   1k   3k   6k  12k  14k  16k  Hz" << endl;
...
{
	ofstream a_file("C:\\Program Files\\EqualizerAPO\\config\\config.txt");

	a_file << "Filter Settings File \n\n";
	a_file << "Equaliser: Generic\n";
	a_file << "No measurement\n\n";
	a_file << "Preamp: " << e[0] << " db\n\n";
	for(int i=1;i<=10;i++)
	{
		a_file << "Filter " << i << ": ON PK     Fc     " << freq[i-1] <<" Hz    Gain  " << e[i] << " db Q 1.0\n";
	}
	a_file.close();
}